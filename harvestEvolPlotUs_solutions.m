...
hold on
Fsols=nan(16,length(Vsrange));
Ssols=nan(16,length(Vsrange));
Fstab=nan(16,length(Vsrange));
for i=1:length(Vsrange)
    Vs=Vsrange(i);
...
for curve=1:size(Ssols,1) %plot each curve (each row of equilibrium values)
    ...
    Ssols_stable=sort(Ssols_stable);
    Ssols_unstable=sort(Ssols_unstable);
    Ssols_stable(logical([zeros(16,1) (abs(diff(Ssols_stable,1,2))>0.2)]))=NaN;
    Ssols_unstable(logical([zeros(16,1) (abs(diff(Ssols_unstable,1,2))>0.2)]))=NaN;
    plot(Vsrange,Ssols_stable(curve,:)'/Smsy,'-k','LineWidth',2); %plot stable parts of the curve
    plot(Vsrange,Ssols_unstable(curve,:)'/Smsy,'--k','LineWidth',2); %plot unstable parts of the curve
end
